...
> 注：本文是阶段性复盘，文中部分参数对应当时实验，不代表当前脚本默认值。当前默认请以 `marl_env/sample_factory_registration.py` 与 `scripts/run_sf_maca_4060_baseline.sh` 为准（当前为 `num_workers=8`、`batch_size=5120`）。

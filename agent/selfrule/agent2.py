...
class Agent(BaseAgent):
    ...
    def get_move_actions(self,agent_id,step_cnt, obs_dict):
        temp_obs=obs_dict['fighter'][agent_id]['info']
        self_pos_x=temp_obs[3]
        self_pos_y=temp_obs[4]
        ...
        # 当 step_cnt 小于 20 时，action 保持为 0
        if step_cnt < 20:
            move_action = 0
        else:
            # 检查是否到了变换方向的步数
            if step_cnt % 50 == 0 and step_cnt != state['last_change_step']:
                # 变换方向，并更新上一次变换方向的步数
                state['current_direction'] = sec.randbelow(359)
                state['last_change_step'] = step_cnt
            # 返回当前方向
            move_action = state['current_direction']
    ...
    def get_action( self, obs_dict, step_cnt):
        ...
        for y in range(10):
            ...
            if obs_dict['fighter'][y]['alive']:
                ...
                move_action = self.get_move_actions(y,step_cnt,obs_dict)
